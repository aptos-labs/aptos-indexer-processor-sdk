...
use aptos_indexer_processor_sdk::{
    ...
    config::processor_mode::ProcessorMode,
    traits::{processor_trait::ProcessorTrait, IntoRunnableStep},
    utils::{chain_id_check::check_or_update_chain_id, starting_version::get_starting_version},
};
use aptos_indexer_processor_sdk_postgres::{
    config::PostgresConfig,
    utils::{
        database::{new_db_pool, run_migrations, ArcDbPool},
        postgres_trait_impls::{
            PostgresChainIdChecker, PostgresProcessorStatusSaver, PostgresStartingVersionRetriever,
        },
    },
};
...
impl ProcessorTrait for EventsProcessor {
    ...
    async fn run_processor(&self) -> Result<()> {
        ...
        let starting_version = get_starting_version(
            ...
            &PostgresStartingVersionRetriever::new(
                self.name(),
        ...
        let processor_status_saver = PostgresProcessorStatusSaver::new(
            self.name(),
            self.config.processor_mode.clone(),
            self.db_pool.clone(),
        );
