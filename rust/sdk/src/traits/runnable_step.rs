        ...
        Output: Send + 'static,
{
    ...
    fn add_input_receiver(self, input_receiver: AsyncReceiver<Vec<Input>>) -> RunnableStepWithInputReceiver<Input, Output, Self> {
...
impl<Input, Output, Step> RunnableStep<Input, Output> for RunnableStepWithInputReceiver<Input, Output, Step>
    where
        ...
        Output: Send + 'static,
        Step: RunnableStep<Input, Output> {
    fn spawn(self, input_receiver: Option<AsyncReceiver<Vec<Input>>>, channel_size: usize) -> (AsyncReceiver<Vec<Output>>, JoinHandle<()>) {
    ...
    fn add_input_receiver(self, _input_receiver: AsyncReceiver<Vec<Input>>) -> RunnableStepWithInputReceiver<Input, Output, Self> {
        panic!("Input receiver already set");
    }
}